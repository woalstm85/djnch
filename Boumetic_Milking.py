                ...
                try:
                    ...
                    with mssql_engine.connect() as mssql_conn:
                        max_milking_id = mssql_conn.execute(
                            text("SELECT ISNULL(MAX(milking_id), 0) FROM TEST_ICT_MILKING_LOG")
                        ).scalar()

                    # PostgreSQL에서 데이터 가져오기
                    with pg_engine.connect() as pg_conn:
                        query = text("""
                            SELECT a.milking_id, to_char(identified_tstamp, 'YYYYMMDD') AS YMD,
                            CASE WHEN to_char(identified_tstamp, 'HH24MMSS') < '120000' THEN '1' ELSE '2' END AS AM_PM,
                            to_char(identified_tstamp, 'HH24MMSS') AS HMS, a.cow_id, b.cow_number, b.cow_name,
                            a.milkingshift_id, detacher_address, id_tag_number_assigned,
                            ...
                            milk_conductivity, cow_activity, convertunits(c.flow_0_15_sec) AS flow_0_15_sec,
                            convertunits(c.flow_15_30_sec) AS flow_15_30_sec,
                            convertunits(c.flow_30_60_sec) AS flow_30_60_sec,
                            convertunits(c.flow_60_120_sec) AS flow_60_120_sec, c.time_in_low_flow,
                            c.reattach_counter, c.percent_expected_milk
                            FROM tblmilkings AS a
                            INNER JOIN public.vewcows AS b ON a.cow_id = b.cow_id
                            INNER JOIN public.tblstallperformances AS c ON a.milking_id = c.milking_id
                            WHERE id_tag_number_assigned <> '' AND a.milking_id > :max_milking_id
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Boumetic Milking_log")
