...
class DataCollectorApp(QMainWindow):
    ...
    def update_status_bar(self):
        try:
            ...
            # DB 연결 상태 표시
            pg_connected = hasattr(self, 'pg_engine') and self.pg_engine is not None
            ms_connected = hasattr(self, 'ms_engine') and self.ms_engine is not None

            self.status_label.setText(f"상태: {'데이터 수집 중' if self.collection_active else '대기 중'} | "
                                      f"CPU: {cpu_percent}% | "
                                      f"DB상태: {'정상' if (pg_connected and ms_connected) else '오류'} | ")

            self.memory_label.setText(f"메모리: {memory_usage:.1f} MB | ")
            self.time_label.setText(f"현재 시간: {QTime.currentTime().toString('HH:mm:ss')}")
