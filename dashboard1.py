...
qt_process_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'PyQt5', 'Qt5', 'bin', 'QtWebEngineProcess.exe')

print(qt_process_path)
...
class MainWindow(QMainWindow):
    ...
    def create_left_layout(self): # 왼쪽 레이아웃 생성 (우사별 사육두수, 차트 등)
        ...
        # v1 우사별 사육두수 테이블
        v1_right_title_label = QLabel("*( )는 저지")
        v1_right_title_label.setFont(QFont("Arial", 10, QFont.Bold))
        v1_right_title_label.setStyleSheet("color: #00c0ff;")
        v1_title_layout.addWidget(v1_right_title_label, alignment=Qt.AlignRight)
    ...
    def v2_data_chart(self, data):
        ...
        fig.add_trace(go.Bar(
            ...
            name='두수',
            marker_color="#007c4c",
            text=[f"{x:,}" for x in v2_chart_data['COW_CNT']],
            textposition='inside',
    ...
    def v3_header(self):     # v3 table data setting
        ...
        # "총 사육 두수" 헤더 및 하위 항목 설정
        self.v3_table.setSpan(0, 9, 1, 3)  # 3개의 열 병합
        ...
        self.v3_table.setItem(0, 9, total_head_title)
